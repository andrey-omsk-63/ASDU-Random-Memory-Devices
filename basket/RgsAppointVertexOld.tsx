...
const RgsAppointVertex = (props: { setOpen: Function; idx: number }) => {
  ...
  const BlurContent = (mode: number, area: number, id: number, funcAr: Function) => {
  ...
  const ChangeId = (
    ...
    if (Number(valueInp) < 9999) {
      ...
      for (let i = 0; i < map.tflight.length; i++) {
        if (map.tflight[i].ID === Number(valueInp) && Number(map.tflight[i].area.num) === AREA) {
  ...
  const InputerId = (valueId: any, funcId: Function, valueAr: number, funcAr: Function) => {
    return (
      <Box sx={styleSetAV}>
