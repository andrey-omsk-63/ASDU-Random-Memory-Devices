...
export const CheckHaveLink = (klu: string, kluLast: string, bindings: any) => {
  console.log('CheckHaveLink:', klu, kluLast, bindings);

  let hv = -1;
  for (let i = 0; i < bindings.tfLinks.length; i++) if (bindings.tfLinks[i].id === kluLast) hv = i;
...
export const MakeMassRoute = (bindings: any, nom: number, map: any, addobj: any) => {
  ...
  // const ExtrId = (bindings: any, nom: number, map: any, addobj: any) => {
  ...
  for (let j = 0; j < massKlu.length; j++) {
    ...
    //console.log('massKlu[j]:', id, massKlu[j], typeof massKlu[j], massKlu[j].length);
    ...
    if (id < 10000) {
      ...
      //console.log('MakeMassRoute:',id,addobj.addObjects)
      for (let i = 0; i < addobj.addObjects.length; i++) {
        if (
          ...
          //have++
          //console.log('ЕСТЬ!!!',addobj.addObjects[i].dgis)
  ...
  if (index < map.tflight.length) {
    ...
    for (let i = 0; i < bindings.tfLinks.length; i++) {
      ...
      if (bindings.tfLinks[i].id === klu) {
        ...
        cont4 = '<br/>Связи:';
        if (recc.north.id) contS = '<br/><b>C:</b> ' + ExtrId(recc.north.id, SL);
        if (recc.add1.id) contSV = '<br/><b>CВ:</b> ' + ExtrId(recc.add1.id, SL);
        if (recc.east.id) contV = '<br/><b>В:</b> ' + ExtrId(recc.east.id, SL);
        if (recc.add2.id) contUV = '<br/><b>ЮВ:</b> ' + ExtrId(recc.add2.id, SL);
        if (recc.south.id) contU = '<br/><b>Ю:</b> ' + ExtrId(recc.south.id, SL);
        if (recc.add3.id) contUZ = '<br/><b>ЮЗ:</b> ' + ExtrId(recc.add3.id, SL);
        if (recc.west.id) contZ = '<br/><b>З:</b> ' + ExtrId(recc.west.id, SL);
        if (recc.add4.id) contSZ = '<br/><b>СЗ:</b> ' + ExtrId(recc.add4.id, SL);
        break;
      }
    }
  } else {
...
export const GetFaza = (mass: any, mas: any, maxFaza: number, kluch: string) => {
  let faza = 0;
  if (mass)
    for (let i = 0; i < mas.length; i++) if (mas[i].id === kluch) faza = Number(mas[i].phase);
  ...
  return (
    <Grid container sx={{ border: 0 }}>
      <Grid item xs={12} sx={styleOutputKey}>
        <Box sx={styleAppSt02}>
          <Box sx={styleOutputKlush}>{klush}</Box> {klush && <Box sx={styleOutputHint}>{dir}</Box>}
        </Box>
      </Grid>
    </Grid>
  );
...
export const SaveСhange = (HAVE: number, have: number, func1: Function, func2: Function) => {
  return (
    <>
      {HAVE > 0 || have > 0 ? (
