...
const RgsAppointVertex = (props: { setOpen: Function; idx: number }) => {
  ...
  const BlurId = (event: any, area: number, id: number, func: Function) => {
    if (!area && !id) return;
  ...
  const InputerArea = (value: number, func: Function, funcId: Function) => {
        ...
        <Grid item xs={5.5} sx={{ fontSize: 14, height: hBlock / 5 }}>
          <Grid container>
            ...
            <Grid item xs sx={{ border: 0 }}>
              <Box sx={styleAppSt02}>
                {InputerArea(valueAr, funcAr, funcId)}
              </Box>
            </Grid>
          </Grid>
          <Grid container>
